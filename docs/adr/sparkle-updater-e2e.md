...
- `SPUUserDriver` dismisses at `showReady(toInstallAndRelaunch:)` after download/extract. Extraction into a temp directory is part of Sparkle’s download path, not install. The macOS fixture must be a real zip of `Dummy.app` (same bundle id and folder name as the host). `tests/e2e/prepare-sparkle-release.mjs` packs that tree with `ditto -c -k` on macOS (Sparkle extracts with `ditto -x -k`) and with a stored ZIP elsewhere. A text file named `.zip` passes EdDSA and then fails Sparkle’s unarchiver.
