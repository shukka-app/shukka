...
2. Put kind-specific collect and version inference in plain ESM next to the script: `scripts/updaters/electron.mjs`, `scripts/updaters/tauri.mjs`, and `scripts/updaters/sparkle.mjs`, plus a small `scripts/updaters/shared.mjs` for shared fail/kind helpers.
...
4. Infer kind from the directory (`yml` → electron; `appcast.xml` or a Sparkle archive+.sig pair → sparkle; `.sig` / `latest.json` / known bundle layout → tauri). Honor optional `SHUKKA_UPDATER_KIND` / Action `updater-kind`. The server still validates with the app’s `updaterKind`.
...
9. Sparkle collector: today’s flat `readdir` (files only). Version from `appcast.xml` then an `App-1.4.2.zip` filename token. Do not recurse. Do not run `generate_appcast` / `sign_update`.
