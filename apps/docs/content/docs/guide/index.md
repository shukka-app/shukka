---
title: 面板入门
description: 初始化、登录、面板结构，以及语言 / 主题 / 视图角色等 per-browser 偏好。
---

## 初始化与登录

首次打开面板会进入 setup 页，设置至少 8 位的管理员密码。之后访问面板任意页面都需要登录；登录态是名为 `shukka_session` 的 cookie（`HttpOnly`、`SameSite=Lax`、14 天）。

Shukka 是单管理员模型：没有注册、没有多用户。忘记密码的恢复路径见[自托管部署](/docs/deployment#忘记密码)。管理员密码可以在设置页修改。

## 面板结构

- **应用列表**：侧栏列出所有 app，头部有新建应用入口。
- **应用详情** `/apps/{appSlug}`：按标签组织——
  ...
  - **API docs**：新标签页打开整页 ReDoc 渲染的 OpenAPI 文档（本站也有一份[在线 API 参考](/api)）。
  - **Settings**：编辑 app 配置（含 Release log 分区）；删除应用仅 admin 角色可见。
- **设置页**：修改管理员密码。

## 语言、主题与视图角色

这三项都是 **per-browser** 偏好，存在 cookie 里，服务端不存任何用户偏好。入口统一在侧栏底部的角色菜单（显示当前角色名的按钮）里：
...
视图角色**只控制面板 UI 入口的可见性**，不是鉴权：服务端不存、不校验角色，直接访问 URL 不被拦截。

| 入口 | content | developer | admin |
|------|---------|-----------|-------|
| 应用列表、Channels 标签（版本表、计数） | ✓ | ✓ | ✓ |
| 趋势图、版本统计、release notes 编辑 | ✓ | | ✓ |
