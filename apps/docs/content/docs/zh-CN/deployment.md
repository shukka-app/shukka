...
Postgres 与 MySQL 是 **opt-in**，**不是** Docker 默认。需要时叠加 overlay：
...
| `SHUKKA_DB_DRIVER` | 未设（`sqlite`） | 元数据适配器。未设或 `sqlite` 为默认。`postgres` / `mysql` 须同时提供对应 URL。**不是 Docker 默认**——镜像与 `compose.yaml` 仍走 SQLite。Worker 不支持。 |
...
| `SHUKKA_DB_URL` | 未设 | `SHUKKA_DB_DRIVER=postgres` 时为 Postgres URL（必填）；`mysql` 时为 MySQL URL（必填）。否则为 Worker / SCF 的远程 libsql HTTP URL。 |
| `SHUKKA_DB_AUTH_TOKEN` | 未设 | 远程 libsql 的可选 token。Postgres / MySQL 不用。 |
