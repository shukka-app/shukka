...
两条路径默认都发布为 **draft**：feed 不变、客户端不可见。确认无误后在面板 promote，或 `PATCH /api/v1/apps/{appSlug}/channels/{channel}` 设 `currentVersion`。需要 CI 一步上线时传 `release: true` / `SHUKKA_RELEASE=true`。语义细节见[发布版本](/docs/guide/publish)。
