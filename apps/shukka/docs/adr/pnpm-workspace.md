...
4. **根**只留工作区清单、lockfile、`.github/`、`LICENSE`、`.gitignore`、产品 README、根 `action.yml`，以及 agent / Docker / act 等平台文件。store 包在 `packages/`（#88）。
5. **Docker**：build context 是工作区根，`file: apps/shukka/Dockerfile`。构建阶段用 pnpm 装 `shukka` 的依赖并 `pnpm build`；运行阶段仍是 Alpine + `.output` + `drizzle/`，`WORKDIR /app`。sqlite 适配器 `boot()` migrate；镜像仍拷贝迁移 SQL。
