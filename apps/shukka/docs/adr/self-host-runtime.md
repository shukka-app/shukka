...
3. **推荐分发**：`apps/shukka/Dockerfile`，Docker context 为工作区根——Debian 构建阶段 `pnpm install --frozen-lockfile --filter shukka...` + `pnpm build`（Nitro 把 libsql 追进 `.output/`），Alpine 运行阶段只拷贝 `.output/` 和 `drizzle/` 到 `WORKDIR /app`。不在运行镜像里再装一份生产 `node_modules`，不含 `apps/docs`。启动时自动 migrate。不需要为本机编译原生 sqlite 绑定。
...
- **Postgres / MySQL 换掉 SQLite 作为默认**：违背「一个容器 + 一个卷」的推荐路径。Postgres / MySQL 是 opt-in 适配器（`SHUKKA_DB_DRIVER=postgres` / `mysql`），默认镜像与 Compose 仍是 sqlite。见 [store-postgres](store-postgres.md)、[store-mysql](store-mysql.md)。
...
- 推荐分发是 `ghcr.io/shukka-app/shukka`（semver 标签发布，见 `docs/adr/ghcr-on-semver-tag.md`）。源码构建入口是工作区根 context + `apps/shukka/Dockerfile`。
