...
- **CI 发布者**：GitHub Action 的 `uses: shukka-app/shukka@v*` 不变。
- **Agent / 文档读者**：内部文档与公开文档站都从本仓库出。

## Goals

1. git 根是 pnpm workspace（`apps/*`、`packages/*`），一把 lockfile。本地用 `ni`；CI 用 `pnpm` 对 lockfile。
2. 当前产品全部进入 `apps/shukka`。根上不留应用源码：`src/`、`tests/`、`public/`、`scripts/`、`skills/`、`drizzle/`、`deploy/`、`Dockerfile`、`wrangler.jsonc`、Vite/vitest/tsconfig、`components.json`、`cloudbaserc.json.example`、内部 `docs/prd|adr|spec`。GitHub Action 的 `action.yml` 留在仓库根，`main` 指向应用内脚本。
...
4. 根只留工作区与平台文件：`package.json` / `pnpm-workspace.yaml` / lockfile、`.github/`、`LICENSE`、`.gitignore`、产品 README、根 `action.yml`。
...
6. GitHub Action 入口留在仓库根：`action.yml` 的 `main` 为 `apps/shukka/scripts/shukka-upload.mjs`。对外 `uses: shukka-app/shukka@v*` 不变。仓库内测试 `uses: ./`。不是 composite stub。
...
- 把 `action.yml` 挪进 `apps/shukka` 并改 `uses:` 路径。
...
根 `action.yml` 执行 `apps/shukka/scripts/shukka-upload.mjs`。

## Failure behavior

- 根上残留应用文件：验收失败（`git ls-files` 根目录不是工作区 + 平台文件）。
- Docker 把 `apps/docs` 打进运行镜像：镜像体积与无关源码泄漏。
- 把 `action.yml` 放进 `apps/shukka`：已有 `uses: shukka-app/shukka@v*` 全部失效。
...
- [x] 仓库内 workflow 使用 `uses: ./`。对外仍是 `uses: shukka-app/shukka@v*`。
...
- Action 入口留在仓库根，`uses: shukka-app/shukka@v*` 不变；脚本在 `apps/shukka`。
