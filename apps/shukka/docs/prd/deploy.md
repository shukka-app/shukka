...
- 不把 Shukka 做成多实例/HA。Postgres / MySQL 是 opt-in 元数据适配器，不是默认，也不随 Docker 镜像起对应数据库进程。
- 不写「如何把桌面应用接到 feed」——那是 shukka-ops，不是本机部署。
- 不提供托管 SaaS。

## 推荐路径

**一台 VPS（或同等单机）上跑 `apps/shukka/Dockerfile`（工作区根 context）构建的容器，挂一个持久卷到 `/data`，前面用 Caddy / nginx 终结 TLS。** 对象存储用 Cloudflare R2、AWS S3 或独立 MinIO；不要和 Shukka 进程抢同一块 ephemeral 盘当制品库。
...
2. 拉公开镜像并运行（容器工作目录是 `/app`，迁移读 `./drizzle`）。没有镜像时在仓库根 `docker build -t shukka -f apps/shukka/Dockerfile .`，把下面的镜像名换成 `shukka`：
...
公开镜像是 `ghcr.io/shukka-app/shukka`（semver 标签发布，见 `docs/prd/container-image.md`）。拉取失败时在仓库根 `docker build -t shukka -f apps/shukka/Dockerfile .`。

同一份编排在 `apps/shukka/deploy/compose.yaml`（Shukka + 示例 MinIO）。钉版本用 `SHUKKA_IMAGE`：
...
可选 Postgres / MySQL overlay（**不是**默认文件；镜像默认仍是 sqlite）：
...
进程的 cwd 必须是 `apps/shukka`（或镜像 `WORKDIR /app`）。SQLite 适配器在 `boot()` 里用打包的迁移 SQL migrate；镜像仍拷贝 `packages/store-sqlite/drizzle` 到 `/app/drizzle`。Postgres 适配器读 `/app/drizzle-postgres`（或包内 `drizzle/`）。MySQL 适配器读 `/app/drizzle-mysql`（或包内 `drizzle/`）。`nr --filter shukka db:generate` / `db:generate:postgres` / `db:generate:mysql` 只在改了对应 schema 之后由开发者执行，生产环境不要跑。
...
`nr --filter shukka juicefs` / `apps/shukka/scripts/juicefs-dev.mjs` 只给本地开发当 S3 网关，不是生产部署方案。
...
Docker 卷默认在 `/data/shukka.db`。Postgres / MySQL 时对同一表跑 `DELETE FROM admin; DELETE FROM sessions;`。删的是密码与登录态，app / channel / 版本记录还在。
...
| `SHUKKA_DB_DRIVER` | unset（`sqlite`） | 元数据适配器。未设或 `sqlite`：文件 / 远程 libsql。`postgres`：Postgres（须同时设 Postgres URL）。`mysql`：MySQL（须同时设 MySQL URL）。其它值拒绝启动。**不是 Docker 默认**；镜像与 `compose.yaml` 仍走 SQLite。Worker 不支持 `postgres` / `mysql`。 |
...
| `SHUKKA_DB_URL` | 未设 | 连接 URL。`SHUKKA_DB_DRIVER=postgres` 时为 Postgres URL（必填）；`mysql` 时为 MySQL URL（必填）。否则为远程 libsql HTTP URL（Worker / SCF 等）。`boot()` 会 migrate。 |
| `SHUKKA_DB_AUTH_TOKEN` | 未设 | 远程 libsql token（可选）。Postgres / MySQL 不用。 |
...
Worker 必填 `SHUKKA_ENCRYPTION_KEY`（不要 filepath）与 `SHUKKA_DB_URL`。登录限速与 feed 命中在 isolate 上关闭，用量看平台日志。人类安装文案在 `apps/docs` 跟进。见 `docs/prd/dual-runtime.md`。
...
升级：拉新镜像或 `git pull && ni && nr --filter shukka build`，停旧进程，同一数据目录启动新进程。启动时若 cwd 下存在 `drizzle/` 会自动 migrate。同一数据目录不要跑两个 Shukka 进程。回滚：换回旧镜像/旧构建，保留数据目录；不要对生产库跑 `db:generate`。
...
| 单机 Docker + 自备 MySQL（`SHUKKA_DB_DRIVER=mysql`） | 可行；MySQL 进程不在默认镜像里，须 overlay 或外部实例 |
...
- [x] 写明 Cloudflare Workers 是第二路径：远程库 + `SHUKKA_ENCRYPTION_KEY`，`pnpm --filter shukka deploy:worker`。
