...
/**
 * The publish skill installs from the nested skill directory, pinned to the
 * commit this server was built from (`__GIT_SHA__` is inlined by vite at
 * build time). The tree URL lets the skills CLI find
 * `apps/shukka/skills/shukka-publish` without a root `skills/` copy. The
 * 'dev' fallback (no git metadata at build time) tracks main instead of a
 * dead URL.
 */
...
npx skills add https://github.com/shukka-app/shukka/tree/${skillRef}/apps/shukka/skills/shukka-publish`,
    },
  }
}
