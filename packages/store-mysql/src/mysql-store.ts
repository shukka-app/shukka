...
function asHitRows(rows: { bucket: unknown; kind: HitBucketRow['kind']; count: unknown }[]): HitBucketRow[] {
  return rows.map((row) => ({
...
export function createMysqlStore(db: Database): Store {
  return {
    async ping() {
    ...
    async createApp(input: CreateAppInput): Promise<StoreResult<App>> {
      try {
        const created = await db.transaction(async (tx) => {
          const [idRow] = await tx.insert(schema.apps).values(input).$returningId()
          const [app] = await tx.select().from(schema.apps).where(eq(schema.apps.id, idRow.id)).limit(1)
    ...
    async updateApp(id, input: UpdateAppInput) {
      await db.update(schema.apps).set(input).where(eq(schema.apps.id, id))
      const [updated] = await db.select().from(schema.apps).where(eq(schema.apps.id, id)).limit(1)
    ...
    async createChannel(appId, name): Promise<StoreResult<Channel>> {
      try {
        const [idRow] = await db.insert(schema.channels).values({ appId, name }).$returningId()
        const [created] = await db.select().from(schema.channels).where(eq(schema.channels.id, idRow.id)).limit(1)
        if (!created) throw new Error('Channel insert returned no row')
        return { ok: true, value: created }
      } catch (error) {
        return conflict(error)
      }
    ...
    async insertVersion(input) {
      try {
        const [idRow] = await db.insert(schema.versions).values(input).$returningId()
        const [row] = await db.select().from(schema.versions).where(eq(schema.versions.id, idRow.id)).limit(1)
    ...
    async updateVersionMetadata(versionId, metadata) {
      await db.update(schema.versions).set({ metadata }).where(eq(schema.versions.id, versionId))
      const [saved] = await db.select().from(schema.versions).where(eq(schema.versions.id, versionId)).limit(1)
      return saved ?? null
    },
    ...
    async finalize(input: FinalizeInput): Promise<StoreResult<Version>> {
      try {
        const created = await db.transaction(async (tx) => {
          const [idRow] = await tx.insert(schema.versions).values({
            appId: input.appId,
            channelId: input.channelId,
            version: input.version,
          ...
          const [version] = await tx.select().from(schema.versions).where(eq(schema.versions.id, idRow.id)).limit(1)
    ...
    async sumHitBucketsForChannel(channelId, since, step): Promise<HitBucketRow[]> {
      const width = sql.raw(String(Math.trunc(step)))
      const bucket = sql<number>`((${schema.hitBuckets.hourStart} div ${width}) * ${width})`
      const rows = await db
        .select({
          bucket,
          kind: schema.hitBuckets.kind,
          count: sql<number>`cast(coalesce(sum(${schema.hitBuckets.count}), 0) as signed)`,
        })
    ...
    async insertApiKey(input): Promise<ApiKey> {
      const [idRow] = await db.insert(schema.apiKeys).values(input).$returningId()
      const [key] = await db.select().from(schema.apiKeys).where(eq(schema.apiKeys.id, idRow.id)).limit(1)
    ...
    async upsertNote(input) {
      ...
      const [note] = await db
        ...
        .from(schema.releaseNotes)
        .where(
          and(eq(schema.releaseNotes.versionId, input.versionId), eq(schema.releaseNotes.locale, input.locale)),
        )
        .limit(1)
